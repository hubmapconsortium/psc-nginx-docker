...
function export_version() {
    export HUBMAP_PSC_NGINX_VERSION=$(tr -d "\n\r" < VERSION | xargs)
    echo "HUBMAP_PSC_NGINX_VERSION: $HUBMAP_PSC_NGINX_VERSION"
}
...
if [[ "$1" != "dev" && "$1" != "prod" ]]; then
    ...
    if [[ "$2" != "config" && "$2" != "build" && "$2" != "start" && "$2" != "stop" && "$2" != "down" ]]; then
        ...
        if [ "$2" = "config" ]; then
            docker-compose -f docker-compose.yml -f docker-compose.$1.yml -p hubmap-psc-nginx config
        elif [ "$2" = "build" ]; then
            # Copy the VERSION file and BUILD file to image
            cp VERSION hubmap-psc-nginx
            cp BUILD hubmap-psc-nginx

            docker-compose -f docker-compose.yml -f docker-compose.$1.yml -p hubmap-psc-nginx build
        elif [ "$2" = "start" ]; then
            docker-compose -f docker-compose.yml -f docker-compose.$1.yml -p hubmap-psc-nginx up -d
        elif [ "$2" = "stop" ]; then
            docker-compose -f docker-compose.yml -f docker-compose.$1.yml -p hubmap-psc-nginx stop
        elif [ "$2" = "down" ]; then
            docker-compose -f docker-compose.yml -f docker-compose.$1.yml -p hubmap-psc-nginx down
        fi
    fi
fi
